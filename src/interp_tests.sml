  ...
  val fact =
    Ast.Lam
      ...
      , Ast.If
          ...
          , Ast.Lit 1
          , Ast.Prim (Ast.Mul, Ast.Var "n", Ast.App
              (Ast.Var "fact", Ast.Prim (Ast.Add, Ast.Lit ~1, Ast.Var "n")))
          )
      )
  ...
  fun testall () =
    let
      ...
      val declist =
        ...
        , Ast.ValRec ("ack", ack)
        , Ast.Val ("ans2", Ast.App
            (Ast.App (Ast.Var "ack", Ast.Lit 3), Ast.Lit 2))
        ]

      val env: Interp.env =
        Dict.empty op= |> Dict.insert "true" (Ast.VInt 1)
